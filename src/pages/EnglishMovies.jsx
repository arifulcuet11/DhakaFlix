...
const SOURCES = ["All", "English Movies", "English Movies 1080p", "IMDb Top 250"];
const PAGE_SIZE = 60;

export default function EnglishMovies() {
  const { movies, loading } = useEnglishMovies();
    ...
    <div className="sg-root">
      {playing && (
        <VideoPlayer
          src={playing.fileUrl}
          title={playing.title}
          subtitle={playing.year ? `${playing.year} · ${playing.quality}` : playing.quality}
      ...
      <div className="sg-header">
        <div className="sg-header-top">
          <div>
            ...
            <h1 className="sg-title">English Movies</h1>
          </div>
        </div>
              ...
              return (
                <div key={m.id} className="sg-card" onClick={() => setPlaying(m)} style={{ cursor: "pointer" }}>
                  <div className="sg-card-thumb">
                    ...
                    {m.source === "IMDb Top 250" && <span className="sg-badge-imdb">IMDb Top 250</span>}
