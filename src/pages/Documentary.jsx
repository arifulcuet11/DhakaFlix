...
export default function Documentary() {
  const { movies, loading } = useDocumentary();
    ...
    <div className="sg-root">
      {playing && (
        <VideoPlayer
          src={playing.fileUrl}
          title={playing.title}
          subtitle={playing.year ? `Documentary · ${playing.year}` : "Documentary"}
      ...
      <div className="sg-header">
        <div className="sg-header-top">
          <div>
            ...
            <h1 className="sg-title">Documentary</h1>
          </div>
        ...
        <div className="sg-count">
          {loading ? "Loading…" : `${filtered.length.toLocaleString()} documentaries`}
      ...
      {loading ? (
        ...
        <>
          ...
          {filtered.length === 0 && (
            <div className="sg-empty">No documentaries found{query ? ` for "${query}"` : ""}</div>
          )}
