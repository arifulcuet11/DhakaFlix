...
const GENRES = ["Animation", "Family", "Adventure", "Comedy", "Fantasy", "Action"];
const PAGE_SIZE = 60;

export default function AnimationMovies() {
  const { movies, loading } = useAnimationMovies();
  ...
  function handleGenre(g) { setActiveGenre(g); setPage(1); updateParam("genre", g === "All" ? "" : g); }
  ...
  const filtered = useMemo(() => {
    let out = movies;
    if (activeGenre !== "All") out = out.filter(m => m.genre?.includes(activeGenre));
    if (query.trim()) {
    ...
    <div className="sg-root">
      {playing && (
        <VideoPlayer
          src={playing.fileUrl}
          title={playing.title}
          subtitle={playing.year ? `Animation · ${playing.year} · ${playing.quality}` : "Animation"}
      ...
      <div className="sg-header">
        <div className="sg-header-top">
          <div>
            ...
            <h1 className="sg-title">Animation Movies</h1>
          </div>
        ...
        <div className="sg-controls">
          ...
          <div className="sg-genres">
            {["All", ...GENRES].map(g => (
              <button key={g} className={`sg-genre-btn${activeGenre === g ? " active" : ""}`} onClick={() => handleGenre(g)}>
                {g}
              </button>
            ))}
          </div>
        ...
        <div className="sg-count">
          ...
          {(activeGenre !== "All" || query) && ` · filtered from ${movies.length.toLocaleString()}`}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          {filtered.length === 0 && (
            <div className="sg-empty">No movies found for "{query || activeGenre}"</div>
          )}
